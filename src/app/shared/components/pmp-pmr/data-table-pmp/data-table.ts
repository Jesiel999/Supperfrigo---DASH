...
import { ListPmp } from '../../../models/financeiro.models';
...
export class DataTablePmpComponent implements OnChanges {
  readonly List = input.required<ListPmp[]>();

  private gridApi!: GridApi<ListPmp>;
  quickFilter = '';

  // ─── Default Col ────────────────────────────────────────────
  readonly defaultCol: ColDef<ListPmp> = {
    sortable:   true,
    filter:     true,
    resizable:  true,
  ...
  // ─── Colunas ────────────────────────────────────────────────
  readonly colunas: ColDef<ListPmp>[] = [
    {
      field:      'codigo_titulo',
      headerName: 'Título',
      width:      80,
    ...
    },
    {
      ...
      width:      200,
      pinned:     'left',
      cellRenderer: (p: ICellRendererParams<ListPmp, string>) =>
    ...
    },
    {
      ...
      width:      150,
      type:       'numericColumn',
      valueFormatter: (p: ValueFormatterParams<ListPmp, number>) =>
        p.value != null
    ...
    },
    {
      ...
      width:           120,
      sort:            'asc',
      valueFormatter:  (p: ValueFormatterParams<ListPmp, string>) =>
        fmtData(p.value),
    ...
    {
      field:      'dias_pagamento',
      headerName: 'Dias Pagamento',
      width:      120,
      type:       'numericColumn',
    },
  ];
  ...
  onGridReady(e: GridReadyEvent<ListPmp>): void {
    this.gridApi = e.api;
